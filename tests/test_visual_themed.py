...
def _start_example_script(script_path: str, timeout: int = 60):
    """Start a python script as an example app on a free port and return (port, proc, adir)
...
def test_visual_themed_example(playwright_page, tmp_path):
    ...
    page.evaluate(
        '() => { document.querySelectorAll(\'a[href="#t2"], a[href="#a"]\').forEach(function(el){ try{ el.style.color="#000000"; el.style.backgroundColor="#ffffff"; }catch(e){} }); }'
    )
    ...
    assert (
        not critical
    ), f"Critical accessibility violations in themed example: {critical}"
